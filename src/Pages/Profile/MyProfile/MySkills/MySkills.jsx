...
const MySkills = () => {
  ...
  // fetching user data
  useEffect(() => {
    fetch(`https://api.geeksofgurukul.com/api/v1/users/userinfo/${user?.email}`)
      .then((res) => res.json())
    ...
    <div className="relative w-4/5 md:min-h-[580px] sm:h-[750px] overflow-hidden overflow-y-scroll mx-auto border border-gray-200 rounded-xl shadow-lg bg-white text-black p-10">
      ...
      <div className="my-5">
        <div className="grid grid-cols-5 gap-4">
          {skills ? (
            ...
            <h3 className="col-span-5 font-poppins font-medium">
              You Didn't Add any skill yet. Please add one first.
            </h3>
          )}
        </div>
      </div>
      ...
      <div className=" my-12">
        <button
          ...
          className="font-poppins font-medium text-center px-2 py-2 bg-green-500 text-white rounded-xl hover:cursor-pointer"
        >
          Add Skills
        </button>
        {editSkill && (
