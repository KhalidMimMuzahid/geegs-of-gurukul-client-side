...
function EachAssignment({
  ...
  const assignmentClick = () => {
    ...
    fetch("http://localhost:5000/assignment-response", {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    fetch(
      `http://localhost:5000/assignmentby_id?_id=${eachAssignment?.assignment_id}`
    )
      .then((res) => res.json())
      .then((data) => {
    ...
    fetch(`http://localhost:5000/assignment-response`, {
      ...
      .then((result) => {
        if (result?.success) {
          ...
          fetch(`http://localhost:5000/assignment-exercises-response`, {
            method: "GET",
            headers: {
              "content-type": "application/json",
              query: JSON.stringify(query),
