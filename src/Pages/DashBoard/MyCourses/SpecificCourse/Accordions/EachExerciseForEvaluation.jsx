...
const EachExerciseForEvaluation = ({
  ...
  const selectedDeadline = selected?.endSAt;
  ...
  useEffect(() => {
    const query = {
      evaluation_id: selected?._id,
      exercise_id: exerciseTemp?.exercise_id,
      studentEmail: user?.email,
    };
    fetch(
      `http://localhost:5000/api/v1/exercises/exercise-response-for-evaluation`,
      {
        method: "GET",
        headers: {
    ...
    if (!isOpen) {
      const exerciseData = {
        ...
        program: {
          ...
          programName: selected?.program?.programName,
        },
        course: {
          course_id: selected?.course?.course_id,
          courseName: selected?.course?.courseName,
        },
        batch: {
          batch_id: selected?.batch?.batch_id,
          batchName: selected?.batch?.batchName,
        },
        module: {
          module_id: selected?.module?.module_id,
          moduleName: selected?.module?.moduleName,
        },
      ...
      fetch(
        ...
        .then((data) => {
          ...
          if (data?.success) {
            ...
            setChangingEvaluationStatus((prev) => {
              const evaluation_id = selected?._id;
              const toggle = !!!prev?.toggle;
              return { evaluation_id, toggle };
            });
  ...
  const onSubmit = async (data) => {
    ...
    const query = {
      ...
      "evaluation.evaluation_id": selected?._id,
...
export default EachExerciseForEvaluation;
